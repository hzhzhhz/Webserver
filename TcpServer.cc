...
void TcpServer::HandleNewConnection(int fd)
{
    ...
    auto tcp_conn {std::make_shared<TcpConnection>(loop,fd)};
    //printf("%s\n","new tcp conn");
    ...
    connections_[tcp_conn->Sockfd()]=tcp_conn;//避免shared_ptr释放
}

void TcpServer::CloseEvent(const std::shared_ptr<TcpConnection> conn)
{
    base_loop->RunInLoop([this,conn]{CloseEventInLoop(conn);});//此时才是断开连接
}
